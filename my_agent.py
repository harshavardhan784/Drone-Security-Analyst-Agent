...
class MyAgentSetup(AgentSetupBase):
...
class MyAgent(AgentBase):
    ...
    def _prepare_system_prompt2(self):
        return """
            ...
            Your responses should be precise, professional, and security-focused. Format your analysis with clear sections, relevant timestamps, and actionable recommendations. When discussing security events, assign appropriate severity levels (Low/Medium/High) based on context and potential risk. Maintain a neutral, analytical tone appropriate for security professionals.
    ...
    def run(self, input_video_path: str, query: str, flag = "detections", output_video_path: str = None, output_csv_path: str = None, Retrieved_logs: str = None):
        
        if flag == "detections":
            df = detect_objects(input_video_path, output_video_path, output_csv_path)

            system_prompt = self._prepare_system_prompt1()
            user_prompt = self._prepare_prompt1(df)
